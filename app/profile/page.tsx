...
// minidenticons opsiyoneldir, eğer hata verirse bu satırı silip aşağıda fallback kullanabilirsin
...
// Avatar URL (Dicebear API - Paket gerektirmez)
    ...
    return (
        <div className="h-[300px] w-full relative overflow-hidden bg-black/20 rounded-xl border border-white/5">
             ...
             {holders.slice(0, 20).map((h, i) => {
                 const size = Math.max(20, Math.min(80, h.percentage * 4)); // Baloncuk boyutu
...
export default function TradePage(props: PageProps) {
  ...
  const handleBurn = () => {
      try {
          ...
          burnContract({ address: tokenAddress, abi: erc20Abi, functionName: "transfer", args: ["0x000000000000000000000000000000000000dEaD", val] });
          toast.success("Burning Tokens! 🔥");
