...
function DarkTokenCard({ tokenAddress }: { tokenAddress: `0x${string}` }) {
  ...
  const handleQuickBuy = (e: any) => {
      e.preventDefault(); e.stopPropagation();
      try {
          writeContract({ address: CONTRACT_ADDRESS, abi: CONTRACT_ABI, functionName: "buy", args: [tokenAddress], value: parseEther("1") });
    ...
    <Link href={`/trade/${tokenAddress}`}>
      ...
      >
        ...
        <div className="flex gap-4 mb-5 items-start">
          <div className="relative shrink-0"><MediaRenderer src={tokenImage} className="w-[60px] h-[60px] rounded-2xl border border-[#FDDC11]/20 object-cover" /></div>
          <div style={{ flex: 1 }}>
            <h3 className="text-lg font-bold text-white mb-1 leading-tight">{name?.toString() || "Loading..."}</h3>
            <div className="flex gap-2 items-center"><span className="text-xs font-bold bg-[#FDDC11]/15 text-[#FDDC11] border border-[#FDDC11]/30 px-2 py-1 rounded-md">{symbol?.toString() || "TKN"}</span></div>
          </div>
        </div>
        <div className="mb-5">
          <div className="flex justify-between mb-2 text-xs"><span className="text-gray-400 font-semibold uppercase tracking-wide">Bonding Curve</span><span className="text-[#FDDC11] font-bold">{realProgress.toFixed(1)}%</span></div>
          <div className="h-2 bg-white/10 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${realProgress}%` }} transition={{ duration: 1.2 }} className="h-full bg-gradient-to-r from-[#FDDC11] to-purple-600 shadow-[0_0_20px_rgba(253,220,17,0.6)]" /></div>
        </div>
        <div className="flex justify-between text-xs text-gray-400 font-semibold">
          <div className="flex items-center gap-1"><Coins size={14} className="text-[#FDDC11]" /><span>{parseFloat(collateralStr).toFixed(2)} MATIC</span></div>
        </div>
...
function LiveTicker() {
  ...
  useWatchContractEvent({ address: CONTRACT_ADDRESS, abi: CONTRACT_ABI, eventName: 'Buy', onLogs(logs: any) { const newEvent = { type: 'BUY', token: logs[0].args.token, amount: parseFloat(formatEther(logs[0].args.amountMATIC)).toFixed(2), buyer: logs[0].args.buyer }; setEvents(prev => [newEvent, ...prev].slice(0, 5)); }});
  return (
...
export default function HomePage() {
  ...
  if (!isMounted) return <div className="min-h-screen bg-[#0a0e27] flex items-center justify-center text-[#FDDC11] font-bold text-xl animate-pulse">Loading AION...</div>;
    ...
    <div className="min-h-screen bg-[#0a0e27] text-white font-sans relative overflow-hidden">
      ...
      <header className="sticky top-0 z-40 bg-[#0a0e27]/90 backdrop-blur-xl border-b border-[#FDDC11]/10">
        <div className="max-w-7xl mx-auto px-5 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-[#FDDC11] to-purple-600 rounded-xl flex items-center justify-center text-black font-black text-xl shadow-[0_0_20px_rgba(253,220,17,0.3)]">A</div>
            <div>
                <div className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-[#FDDC11] to-purple-500 bg-clip-text text-transparent">AION</div>
                <div className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">Meme Launchpad</div>
            </div>
          </div>
          <div className="flex gap-3 items-center">
            <Link href="/swipe" className="hidden md:flex p-2.5 bg-[#FDDC11]/10 text-[#FDDC11] rounded-xl font-bold items-center gap-2 hover:bg-[#FDDC11]/20 transition-all border border-[#FDDC11]/20"><Flame size={18}/> Tinder Mode</Link>
            <button onClick={() => setIsModalOpen(true)} className="bg-gradient-to-r from-[#FDDC11] to-purple-600 text-black border-none px-5 py-2.5 rounded-xl font-bold text-sm cursor-pointer transition-all hover:scale-105 shadow-[0_0_20px_rgba(253,220,17,0.3)] flex items-center gap-2"><Rocket size={18} /> LAUNCH</button>
      ...
      <main className="max-w-7xl mx-auto px-5 py-10 relative z-10">
        ...
        <div className="mb-10 relative max-w-lg mx-auto">
            ...
            <input type="text" placeholder="Search tokens by address..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#0f1225] border border-white/10 rounded-2xl py-4 pl-12 pr-6 text-white outline-none focus:border-[#FDDC11] transition-colors shadow-xl" />
        </div>

        <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide justify-center">
            {['Trending', 'New', 'Gainers', 'Volume'].map(t => (
                <button key={t} onClick={() => setActiveTab(t.toLowerCase())} className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${activeTab === t.toLowerCase() ? 'bg-gradient-to-r from-[#FDDC11] to-purple-600 text-black shadow-lg scale-105' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white border border-white/5'}`}>{t}</button>
            ))}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredTokens.length > 0 ? (filteredTokens.map((addr) => <DarkTokenCard key={addr} tokenAddress={addr as `0x${string}`} />)) : (<div className="col-span-full text-center py-20 text-gray-500 text-xl font-bold">No tokens found yet. Be the first to launch! 🚀</div>)}
        </div>
      </main>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-md bg-[#0a0e27] rounded-3xl border border-[#FDDC11]/20 shadow-[0_0_50px_rgba(253,220,17,0.15)] overflow-hidden">
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-[#FDDC11]/10 to-transparent">
              <div className="flex items-center gap-3"><div className="p-2 bg-[#FDDC11]/20 rounded-lg"><Rocket size={24} className="text-[#FDDC11]" /></div><span className="font-black text-xl tracking-tight">Launch Token</span></div>
            ...
            <div className="p-6 flex flex-col gap-5 max-h-[80vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Name</label><input type="text" placeholder="Bitcoin" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-[#FDDC11] transition-colors font-bold" /></div>
                <div><label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Ticker</label><input type="text" placeholder="BTC" maxLength={10} value={formData.ticker} onChange={(e) => setFormData({...formData, ticker: e.target.value.toUpperCase()})} className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-[#FDDC11] transition-colors font-bold" /></div>
              </div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Description</label><textarea placeholder="Tell us about your token..." value={formData.desc} onChange={(e) => setFormData({...formData, desc: e.target.value})} className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none h-24 resize-none focus:border-[#FDDC11] transition-colors text-sm" /></div>
              <div>
                <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Image / Video URL</label>
                <div onClick={() => fileInputRef.current?.click()} className="w-full p-6 bg-white/5 border-2 border-dashed border-white/10 rounded-xl cursor-pointer text-center hover:border-[#FDDC11] hover:bg-[#FDDC11]/5 transition-all flex flex-col items-center gap-2 group">
                  <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*,video/*" className="hidden" />
                  {previewUrl ? <MediaRenderer src={previewUrl} className="w-24 h-24 object-cover rounded-xl shadow-lg" /> : <><div className="p-3 bg-white/5 rounded-full group-hover:bg-[#FDDC11]/20 transition-colors"><Upload size={24} className="text-gray-400 group-hover:text-[#FDDC11]" /></div><span className="text-xs text-gray-500 font-bold group-hover:text-[#FDDC11]">Click to upload</span></>}
                </div>
                <input type="text" placeholder="Or paste URL here..." value={formData.image} onChange={(e) => { setFormData({...formData, image: e.target.value}); setPreviewUrl(e.target.value); }} className="w-full mt-3 p-3 bg-white/5 border-none rounded-xl text-xs text-white outline-none focus:ring-1 focus:ring-[#FDDC11]" />
              </div>
              <div className="grid grid-cols-3 gap-2">
                <input type="text" placeholder="Twitter" value={formData.twitter} onChange={(e) => setFormData({...formData, twitter: e.target.value})} className="p-3 bg-white/5 border border-white/10 rounded-xl text-xs text-white outline-none focus:border-[#FDDC11]" />
                <input type="text" placeholder="Telegram" value={formData.telegram} onChange={(e) => setFormData({...formData, telegram: e.target.value})} className="p-3 bg-white/5 border border-white/10 rounded-xl text-xs text-white outline-none focus:border-[#FDDC11]" />
                <input type="text" placeholder="Web" value={formData.website} onChange={(e) => setFormData({...formData, website: e.target.value})} className="p-3 bg-white/5 border border-white/10 rounded-xl text-xs text-white outline-none focus:border-[#FDDC11]" />
              </div>
              <button onClick={handleCreate} disabled={isPending || isConfirming} className="w-full p-4 bg-gradient-to-r from-[#FDDC11] to-orange-500 text-black rounded-xl font-black text-sm hover:scale-[1.02] active:scale-95 transition-all shadow-lg disabled:opacity-70 disabled:scale-100">{isPending ? "CONFIRMING..." : isConfirming ? "DEPLOYING..." : "CREATE & LAUNCH (0.1 MATIC)"}</button>
              <div className="text-center text-[10px] text-gray-500 font-bold uppercase tracking-widest">Cost: 0.1 MATIC • Instant Trading</div>
            </div>
          </motion.div>
        </div>
      )}
