...
export default function TradePage({ params }: { params: { id: string } }) {
  ...
  const [isApproving, setIsApproving] = useState(false); // Yeni State
  ...
  const handleApprove = async () => {
      try {
          ...
          writeContract({ 
              ...
              args: [CONTRACT_ADDRESS, maxUint256] // Sonsuz onay
          });
  ...
  const handleTx = (type: "buy" | "sell" | "burn") => {
    ...
    try {
        ...
        // YAKIM İŞLEMİ (BURN) - 0x...dEaD adresine transfer
        if (type === "burn") {
            writeContract({ 
                address: tokenAddress, 
                abi: erc20Abi, 
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const fromBlock = blockNumber - 1000n; // Son 1000 blok
  ...
  // Handle Percentage Input (SAFE CALCULATION)
  const handlePercentage = (percent: number) => {
    if(activeTab === "buy") {
        const bal = maticBalance ? parseFloat(maticBalance.formatted) : 0;
        // Gaz ücreti için 0.05 MATIC ayır
        const safeBal = Math.max(0, bal - 0.05);
        setAmount((safeBal * (percent/100)).toFixed(4));
    } else {
        const bal = userTokenBalance ? parseFloat(formatEther(userTokenBalance as bigint)) : 0;
        // Satışta/Yakımda %100 seçilirse hata olmaması için %99.9'unu al (Rounding error fix)
