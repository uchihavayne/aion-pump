...
import { ArrowLeft, Wallet, Star, TrendingUp, Coins, Copy, LogOut, Trophy, Shield, Award, Edit2, Check, X } from "lucide-react";
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px', position: 'relative', zIndex: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '24px', flex: 1 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ marginBottom: '8px' }}>
                   {isEditing ? (
                       ...
                       <div style={{ display: 'flex', flexDirection: 'column' }}>
                           ...
                           {/* Rütbe Rozeti (İsim Varsa Altta Çıkar) */}
                ...
                <div style={{ fontSize: '14px', color: '#94a3b8', marginBottom: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }} onClick={() => { navigator.clipboard.writeText(address || ''); toast.success("Copied!"); }}>
            ...
            <div style={{ display: 'flex', gap: '40px', textAlign: 'right' }}>
                <div><div style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '600', marginBottom: '4px', textTransform: 'uppercase' }}>Net Worth</div><div style={{ fontSize: '28px', fontWeight: '900', color: '#10b981' }}>${totalValue.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div></div>
                <div><div style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '600', marginBottom: '4px', textTransform: 'uppercase' }}>Portfolio</div><div style={{ fontSize: '28px', fontWeight: '900', color: '#fff' }}>{heldTokens.length} <span style={{fontSize:'16px', color:'#64748b'}}>Tokens</span></div></div>
            </div>
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '80px 20px', borderRadius: '20px', border: '1px dashed rgba(253, 220, 17, 0.2)', backgroundColor: 'rgba(253, 220, 17, 0.05)' }}>
            ...
            <p style={{ fontSize: '16px', color: '#64748b', margin: 0 }}>
              {activeTab === "held" ? "You don't hold any tokens yet. Start trading!" : "Your watchlist is empty."}
            </p>
