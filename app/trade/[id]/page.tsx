...
import { useParams } from "next/navigation"; // FIX: Params için en güvenli yöntem
import { 
...
export default function TradePage() {
  // FIX: useParams kullanıyoruz, props değil!
  const params = useParams();
  const id = params?.id as string;
  const tokenAddress = id ? (id as `0x${string}`) : "0x0000000000000000000000000000000000000000"; // Fallback address
  ...
  const fetchDataEngine = async () => {
    ...
    try {
      ...
      const sortedHolders = Object.entries(balances).filter(([_, bal]) => bal > 100n).sort(([, a], [, b]) => (b > a ? 1 : -1)).map(([addr, bal]) => ({ address: addr, percentage: (Number(bal) * 100) / 1_000_000_000 / 10**18 }));
      ...
      allEvents.forEach((event: any) => {
        ...
        newTrades.unshift({ user: event.args.buyer || event.args.seller, type: event.type, maticAmount: mVal.toFixed(4), tokenAmount: tVal.toFixed(2), price: price.toFixed(8) });
        newChart.push({ name: event.blockNumber.toString(), price: price, fill: event.type === 'BUY' ? '#10b981' : '#ef4444' });
        lastP = price;
      });
  ...
  }, [id, publicClient, userTokenBalance]); 
